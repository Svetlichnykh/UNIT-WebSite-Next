        ...
        {galleryData.map(({ id, img, img_desc, img_title, link }) => {
          return (
            <div
              ...
              onMouseLeave={() => setExpendItem(expendItem)}
              className={` ${expendItem === id ? "lg:basis-[47%] basis-[50%]" : "lg:basis-[20%] basis-[30%]"} flex-grow sm:h-[750px] h-[420px] overflow-hidden group transition-all duration-700 relative`}
            >
              <div className='absolute w-full h-full top-0 left-0 flex flex-col justify-between 2xl:pl-[30px] pl-5 pr-5 2xl:pr-0 py-[30px] after:absolute after:left-0 after:bottom-0 after:contents-[""] after:w-full after:h-1/2 after:bg-bottom-liner after:z-[-1] z-10'>
                <h3
                  className={`text-6xl font-extrabold leading-120 transition-all duration-700 text-transparent webkit-text-stroke-width-1 webkit-text-stroke-white ${expendItem === id ? "webkit-text-stroke-primary" : ""}`}
                >
                  {id}
                </h3>
                <div
                  className={`flex 2xl:flex-row ${expendItem === id ? "flex-col" : "flex-col sm:flex-row"} items-start justify-between 2xl:items-end`}
                >
                  <Link
                    href={link}
