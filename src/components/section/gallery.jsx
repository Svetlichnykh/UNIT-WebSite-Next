            ...
            <div className='flex flex-col sm:flex-row lg:flex-nowrap flex-wrap lg:pt-30 2sm:pt-20 pt-14'>
                {
                    galleryData.map(({ id, img, img_desc, img_title, link }) => {
                        return (
                            <div key={id} onMouseEnter={() => setExpendItem(id)} onMouseLeave={() => setExpendItem(expendItem)} className={` ${expendItem === id ? "lg:basis-[47%] basis-[50%]" : "lg:basis-[20%] basis-[30%]"} flex-grow sm:h-[750px] h-[420px] overflow-hidden group transition-all duration-700 relative`}>
