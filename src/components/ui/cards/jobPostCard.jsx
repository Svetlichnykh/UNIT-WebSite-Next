    ...
    <li className="bg-secondary grid lg:grid-cols-[8%_25%_20%_20%_auto] lg:gap-y-0 gap-y-6 justify-between items-center mb-7.5 py-8 px-7.5 border border-primary">
      ...
      <div>
        ...
        <p className="text-primary-foreground">
          ...
          <span className="font-bold whitespace-nowrap">{date_line}</span>{" "}
        </p>
      </div>
      <div>
        ...
        <p className="text-primary-foreground flex gap-2">
          ...
          <span className="font-bold">{salary.time}</span>
        </p>
      </div>
      <Link href={`/${link}`}>
        {" "}
        <ButtonFill className={"px-7.5 py-2.5"}>Apply Now</ButtonFill>{" "}
      </Link>
    </li>
  );
};
